...
def load_model(path, dataset, K, budget, run_id):
    ...
    if dataset == 'camera1' or dataset == 'monitor':
        EmbModel.load_state_dict(torch.load(os.path.join('checkpoints', dataset, str(K), budget, str(run_id), 'last_blocker_model.pt')))
    else:
        EmbModel.load_state_dict(torch.load(os.path.join('checkpoints', dataset_category, dataset, str(K), budget, str(run_id), 'last_blocker_model.pt')))
...
dataset = sys.argv[1]
if dataset == 'AB':
    path = '../data4/ER-Magellan/Textual'
elif dataset == 'camera1' or dataset == 'monitor':
    path = '../data4/Alaska'
else:
    path = '../data4/ER-Magellan/Structured'
    ...
    for run_id in range(5):
        try:
            ...
            for k in [1,2,5,10,20,50]:
                ...
                # ratio = cal_posrate(topkA, gt)
                # ratio_list.append(str(ratio))
            # print(' '.join(recall_list))
            # print(' '.join(ratio_list))
            recall_list_all.append(recall_list)
        except:
            pass
    recall_avg = np.mean(recall_list_all, 0)
    print(' '.join([str(v) for v in recall_avg]))
    recall_avg_dict[budget] = recall_avg

for k, recall_avg in recall_avg_dict.items():
    print(sys.argv[1], sys.argv[2], k, ' '.join([str(v) for v in recall_avg]))