...
class SingleEntityDataset(data.Dataset):
    ...
    def combine_token_feature_text(self):
        ...
        for entity in self.data:
            ...
            entity_text.append(
                self.attr2text(list(self.attr_list.copy()), list(entity))
            )
        return entity_text
...
def read_entity(dataset, table=None, shuffle=True):
    ...
    if table is None:
        ...
        if "wdc" not in dataset:
            df = df.append(
                pd.read_csv(os.path.join(dataset, "tableB.csv"), sep=",", index_col=0)
            )
    else:
...
def load_gt(path, dataset):
    gt = pd.read_csv(
        os.path.join(path, dataset, "matches.csv"), index_col=0
...
def get_SentEmb(EmbModel, dataset):
    ...
    data_iter = data.DataLoader(
        dataset=dataset, batch_size=256, shuffle=False, num_workers=0, collate_fn=padder
    )
    EmbModel.eval()
    # get emb
    emb = []
...
def get_emb(path, dataset, K, budget, run_id):
    attr_listA, entity_listA = read_entity(
        os.path.join(path, dataset), table="tableA", shuffle=False
    )
    if "wdc" in dataset:
        attr_listB, entity_listB = read_entity(
            os.path.join(path, dataset), table="tableA", shuffle=False
        )
    else:
        attr_listB, entity_listB = read_entity(
            os.path.join(path, dataset), table="tableB", shuffle=False
        )
    datasetA = SingleEntityDataset(
        entity_listA, attr_listA, lm="sent-bert", max_len=128, add_token=True
    )
    datasetB = SingleEntityDataset(
        entity_listB, attr_listB, lm="sent-bert", max_len=128, add_token=True
    )
    EmbModel = load_model(path, dataset, K, budget, run_id)
...
def cal_recall(topkA, gt, test_idxs):
    ...
    df = pd.DataFrame(
        {
            "ltable_id": ltable_ids,
            "rtable_id": rtable_ids,
            "flag": [1] * len(ltable_ids),
        }
    )
    df = df.set_index(["ltable_id", "rtable_id"])
    df = gt.join(df)
...
if __name__ == "__main__":
    ...
    test_idxs = pd.read_csv(
        os.path.join(path, dataset, "test_idxs.csv"), index_col=0
    ).values
    recall_avg_dict = {}
    budget = hp.total_budget
    run_id = hp.run_id
    recall_list_all = []

    embeddingA, embeddingB = get_emb(path, dataset, K, str(budget), run_id)
    recall_list = []
    ratio_list = []
    ...
    print(
        ...
        hp.topK,
        hp.total_budget,
        "run_id_" + str(hp.run_id),
        " ".join([str(v) for v in recall_list]),
    )
