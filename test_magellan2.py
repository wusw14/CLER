...
def load_matcher(hp, iter):
    model = DittoConcatModel(lm = hp.lm)
    model = model.cuda()
    optimizer = AdamW(model.parameters(), lr=hp.lr)
    if hp.fp16:
        model, optimizer = amp.initialize(model, optimizer, opt_level='O2')
    model.load_state_dict(torch.load(os.path.join('checkpoints', hp.dataset, str(hp.topK), str(hp.total_budget), str(hp.run_id), 'Iter'+str(iter)+'_matcher_model.pt')))
    print(os.path.join('checkpoints', hp.dataset, str(hp.topK), str(hp.total_budget), str(hp.run_id), 'Iter'+str(iter)+'_matcher_model.pt'), 'loaded!!!')
    return model
...
if __name__=="__main__":
    ...
    for iter in range(11):
        ...
        all_f1_list.append(f1*100)
        # all_pre_list.append(pre)
        # all_rec_list.append(rec)
        print(iter, f1)

# print(hp.topK, 'Final Test F1', hp.total_budget, np.nanmean(all_f1_list))
# print(hp.topK, 'Final Test Precision', hp.total_budget, np.nanmean(all_pre_list))
# print(hp.topK, 'Final Test Recall', hp.total_budget, np.nanmean(all_rec_list))

for i in range(11):
    print(data_abbr, hp.total_budget, i, all_f1_list[i])